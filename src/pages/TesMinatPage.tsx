...
const TesMinatPage: React.FC = () => {
  ...
  const calculateResults = () => {
    ...
    const topScoreKey = Object.keys(scores).reduce((a, b) =>
  ...
  const progress =
    ((currentQuestionIndex + 1) / INTEREST_QUESTIONS.length) * 100;
  ...
  const Intro = () => (
    <div className="text-center bg-white p-10 rounded-2xl shadow-xl max-w-2xl mx-auto animate-boing-in">
      ...
      <p className="text-gray-600 mb-8">
        Jawab {INTEREST_QUESTIONS.length} pertanyaan singkat ini untuk memahami
        diri Anda lebih dalam. Jawablah sejujurnya sesuai kata hati untuk hasil
        yang akurat. Tidak ada jawaban benar atau salah.
      </p>
      <button
        onClick={() => setStep("test")}
        className="bg-teal-600 text-white font-bold py-3 px-8 rounded-full text-lg hover:bg-teal-700 transition-colors shadow-lg"
      >
        Mulai Tes
      </button>
    </div>
  ...
  const Test = () => (
    <div className="bg-white p-8 rounded-2xl shadow-xl max-w-2xl mx-auto">
      ...
      <p className="text-sm text-gray-500 mb-4">
        Pertanyaan {currentQuestionIndex + 1} dari {INTEREST_QUESTIONS.length}
      </p>
      ...
      <div className="text-center mt-8">
        <button
          ...
          className="bg-stone-500 text-white font-bold py-2 px-6 rounded-full hover:bg-stone-600 disabled:bg-gray-400 shadow"
        >
          {currentQuestionIndex < INTEREST_QUESTIONS.length - 1
            ? "Lanjut"
            : "Lihat Hasil"}
        </button>
      </div>
    ...
    return (
      <div className="bg-white p-6 md:p-10 rounded-2xl shadow-xl max-w-4xl mx-auto animate-boing-in">
        ...
        <div className="text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-teal-600 my-2">
            {finalResult.profile}
          </h1>
        </div>

        <p className="text-gray-700 text-base md:text-lg text-center max-w-3xl mx-auto mt-4 mb-8">
        ...
        <div className="my-8">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {majors.map((jurusan) => (
              <div
                key={jurusan.id}
                className="bg-gray-100 p-4 rounded-lg border border-gray-200 hover:bg-teal-50 transition-colors"
              >
        ...
        <div className="mt-10 pt-6 border-t border-gray-200 text-center space-y-4 md:space-y-0 md:flex md:justify-center md:space-x-4">
          <Link
            to="/jurusan"
            className="w-full md:w-auto inline-block bg-gray-700 text-white font-bold py-3 px-8 rounded-full text-lg hover:bg-gray-800 transition-colors"
          >
            Eksplorasi Semua Jurusan
          </Link>
          <Link
            to="/konsultasi"
            className="w-full md:w-auto inline-block bg-teal-600 text-white font-bold py-3 px-8 rounded-full text-lg hover:bg-teal-700 transition-colors"
          >
