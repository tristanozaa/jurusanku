...
const JurusanCard: React.FC<{
  jurusan: Jurusan;
  onSelect: (jurusan: Jurusan) => void;
  index: number;
}> = ({ jurusan, onSelect, index }) => (
  <div
...
const PremiumDataBlock: React.FC<{ jurusan: Jurusan }> = ({ jurusan }) => (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
    <div className="bg-gray-100 p-3 rounded-lg">
      <strong>Proyeksi Pertumbuhan Karir:</strong> {jurusan.proyeksiPertumbuhan}
      %/tahun
    </div>
    <div className="bg-gray-100 p-3 rounded-lg">
      <strong>Jumlah Lapangan Kerja:</strong>{" "}
      {jurusan.jumlahLapanganPekerjaan.toLocaleString("id-ID")}
    </div>
    <div className="bg-gray-100 p-3 rounded-lg">
      <strong>Waktu Rata-Rata Dapat Kerja:</strong>{" "}
      {jurusan.waktuDapatKerjaBulan} bulan
    </div>
  </div>
);
...
const PremiumLock: React.FC = () => (
  <div className="relative blur-sm select-none">
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
      <div className="bg-gray-100 p-3 rounded-lg">
        <strong>Proyeksi Pertumbuhan Karir:</strong> 12%/tahun
      </div>
      <div className="bg-gray-100 p-3 rounded-lg">
        <strong>Jumlah Lapangan Kerja:</strong> 150.000
      </div>
      <div className="bg-gray-100 p-3 rounded-lg">
        <strong>Waktu Rata-Rata Dapat Kerja:</strong> 5 bulan
      </div>
    </div>
  </div>
);

const JurusanModal: React.FC<{ jurusan: Jurusan; onClose: () => void }> = ({
  jurusan,
  onClose,
}) => {
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="space-y-3">
                {Object.entries(jurusan.kurikulum).map(
                  ([semester, matkuls]) => (
                    <div key={semester}>
                        ...
                        {matkuls.map((item, index) => {
                          return (
                            <button
                              ...
                              className={`text-left text-sm font-medium px-3 py-1 rounded-full transition-all duration-200 ${
                                selectedMatkul?.nama === item.nama
                                  ? "bg-teal-600 text-white scale-105"
                                  : "bg-teal-100 text-teal-800 hover:bg-teal-200 focus:outline-none focus:ring-2 focus:ring-teal-500"
                              }`}
                            >
                              {item.nama}
                            </button>
            ...
            <div>
              <h4 className="font-semibold text-lg text-gray-800 mb-2">
                Prospek Karir & Pendapatan
              </h4>
              <div className="bg-green-100 p-3 rounded-lg text-sm mb-4">
                <strong>Estimasi Gaji Awal:</strong>{" "}
                {formatRupiah(jurusan.gajiRataRata)}/bulan
              </div>
                  ...
                  <>
                    ...
                    <div className="absolute inset-0 flex flex-col justify-center items-center bg-white/80 rounded-lg">
                      <div className="text-center p-4">
                        <h4 className="font-bold text-lg text-stone-600">
                          Fitur Premium
                        </h4>
                        <p className="text-gray-700 mb-3 text-sm max-w-sm">
                          Upgrade untuk melihat data eksklusif: Proyeksi Karir,
                          Jumlah Lowongan, dan estimasi waktu mendapat kerja.
                        </p>
                        <Link
                          to="/account"
                          onClick={onClose}
                          className="bg-stone-500 text-white font-bold py-2 px-4 rounded-full hover:bg-stone-600 transition-colors"
                        >
                          Buka Fitur Premium
                        </Link>
                      </div>
            ...
            >
              ...
              >
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M21.582,6.186C21.328,5.242,20.758,4.672,19.814,4.418C18.109,4,12,4,12,4S5.891,4,4.186,4.418C3.242,4.672,2.672,5.242,2.418,6.186C2,7.891,2,12,2,12s0,4.109,0.418,5.814c0.254,0.944,0.824,1.514,1.768,1.768C5.891,20,12,20,12,20s6.109,0,7.814-0.418c0.944-0.254,1.514-0.824,1.768-1.768C22,16.109,22,12,22,12S22,7.891,21.582,6.186z M10,15.464V8.536L16,12L10,15.464z"
                ></path>
              </svg>
              Tonton di YouTube
            </a>
...
const JurusanPage: React.FC = () => {
  ...
  const faculties = useMemo(
    () => ["All", ...[...new Set(JURUSAN_DATA.map((j) => j.fakultas))].sort()],
    []
  );

  const filteredJurusan = JURUSAN_DATA.filter((j) => {
    const matchesSearch = j.nama
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    const matchesCategory =
      selectedCategory === "All" || j.kategori === selectedCategory;
    const matchesFakultas =
      selectedFakultas === "All" || j.fakultas === selectedFakultas;
    ...
    <div className="space-y-8">
      <div className="text-center animate-boing-in">
        <h1 className="text-4xl font-extrabold">
          Eksplorasi Ratusan Jurusan Kuliah
        </h1>
        <p className="text-gray-600 mt-2">
          Temukan informasi lengkap mengenai jurusan impian Anda untuk keputusan
          yang lebih matang.
        </p>
      </div>

      <div
        className="sticky top-16 bg-stone-50/80 backdrop-blur-md py-4 z-10 space-y-4 animate-boing-in"
        ...
        {filteredJurusan.length > 0 ? (
          filteredJurusan.map((jurusan, index) => (
            <JurusanCard
              key={jurusan.id}
              jurusan={jurusan}
              onSelect={setSelectedJurusan}
              index={index}
            />
          ))
        ) : (
          <p className="text-center text-gray-500 col-span-full py-10">
            Maaf, jurusan yang Anda cari tidak ditemukan. Coba kata kunci lain.
          </p>
        )}
      </div>
