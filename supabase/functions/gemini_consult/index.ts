...
serve(async (req) => {
  ...
  if (req.method === "OPTIONS") {
    return new Response("ok", {
      headers: {
        "Access-Control-Allow-Origin": "https://jurusanku.netlify.app", // Di produksi, ganti dengan URL web Anda
  ...
  try {
    ...
    const { message, history, interestProfile, recommendedMajors } =
      await req.json();
    ...
    if (recommendedMajors && recommendedMajors.length > 0) {
      const majorNames = (recommendedMajors as Jurusan[])
        .map((j) => j.nama)
        .join(", ");
    ...
    return new Response(JSON.stringify({ reply: text }), {
      headers: {
        ...
        "Access-Control-Allow-Origin": "https://jurusanku.netlify.app", // Sesuaikan di produksi
      },
    });
  } catch (error) {
